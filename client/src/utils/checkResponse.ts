...
const isSendPossible = (
  IDResponseText: string,
  userNameResponseText: string,
  passwordResponseText: string,
): boolean => {
  if (IDResponseText === '' && userNameResponseText === '' && passwordResponseText === '')
    return true;
  return false;
};

export { checkLogin, validateID, validateUserName, validatePassword, validateForm, isSendPossible };
